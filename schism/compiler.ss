...
(library (schism compiler)
    ...
    (expand-macros
	 ...
	 (if (zero? n)
       ...
       (define (make-string n)
         (%set-tag (make-vector n #\0) ,(string-tag)))
  ...
  ;; definitions.
  (define (add-top-level* env name args)
    (add-env name
             (lambda ()
  ...
  (define (add-top-level env def)
    (let ((tag (car def)))
      (cond
       ((eq? tag 'define)
        (add-top-level* env (caadr def) (cdadr def)))
       ((eq? tag '%wasm-import)
        (add-top-level* env (caaddr def) (cdaddr def)))
       (else
        (error 'add-top-level "unmatched top-level declaration")))))
  (define (add-intrinsic env intrinsic)
    (let ((name (car intrinsic)) (args (cdr intrinsic)))
      (add-env name
               (lambda ()
                 (let ((args (map rename-var args)))
                   `(lambda ,args (icall ,name . ,(lexical-refs args)))))
               env)))
  (define (add-intrinsic-relop env intrinsic)
    (let ((name (car intrinsic)) (args (cdr intrinsic)))
      (add-env name
               (lambda ()
                 (let ((args (map rename-var args)))
  ...
  (define (add-relops relops env)
    (fold-left add-intrinsic-relop env relops))
      ...
      (cond
       ((eq? tag 'let)
        (let* ((vars (map car (cadr expr)))
          ...
          `(let ,bindings ,body)))
       ((eq? tag 'begin)
        (cons 'begin (map simplify-expr (cdr expr))))
       ((eq? tag 'if)
    ...
    (if (and (pair? expr) (symbol? (car expr)))
        (let ((tag (car expr)))
          (cond
           ...
           ((eq? tag 'begin)
            (cons 'begin (map apply-representation-expr (cdr expr))))
           ((eq? tag 'var) expr)
  ...
  (define (compile-bindings bindings env index)
	...
	#f
        (let ((val (compile-expr (cadar bindings) env))
              ...
              (tail (compile-bindings (cdr bindings) env (+ 1 index))))
          (if tail
              `(begin ,val ,tail ,init)
              `(begin ,val ,init)))))
