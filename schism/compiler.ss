  ...
  (define (replace-tail-calls-begin expr tail?)
    (cond
     ...
     (else (cons (replace-tail-calls-expr (car expr) #f)
		 (replace-tail-calls-begin (cdr expr) tail?)))))
  (define (replace-tail-calls-expr expr tail?)
    (let ((tag (car expr)))
      (cond
       ((or (literal? expr) (eq? tag 'var) (eq? tag '%function-index)) expr)
       ((or (intrinsic? tag) (relop? tag))
	(cons tag (map (lambda (e) (replace-tail-calls-expr e #f)) (cdr expr))))
       ((eq? tag 'call)
	(if tail?
	    `(tail-call ,(cadr expr) . ,(map (lambda (e) (replace-tail-calls-expr e #f))
					     (cddr expr)))
	    `(,tag ,(cadr expr) . ,(map (lambda (e) (replace-tail-calls-expr e #f))
					(cddr expr)))))
       ((eq? tag 'apply-procedure)
	(if tail?
	    `(tail-apply-procedure . ,(map (lambda (e) (replace-tail-calls-expr e #f))
					   (cdr expr)))
	    `(,tag . ,(map (lambda (e) (replace-tail-calls-expr e #f))
			   (cdr expr)))))
       ((eq? tag 'apply-procedure)
	`(,tag . ,(map (lambda (e) (replace-tail-calls-expr e #f)) (cdr expr))))
       ((eq? tag 'if)
	`(,tag ,(replace-tail-calls-expr (cadr expr) #f)
	       ,(replace-tail-calls-expr (caddr expr) tail?)
	       ,(replace-tail-calls-expr (cadddr expr) tail?)))
       ((eq? tag 'begin)
	(cons tag (replace-tail-calls-begin (cdr expr) tail?)))
       ((eq? tag 'let)
	`(,tag ,(map (lambda (binding) `(,(car binding)
					 ,(replace-tail-calls-expr (cadr binding) #f)))
		     (cadr expr))
	       ,(replace-tail-calls-expr (caddr expr) tail?)))
       (else (begin (trace-value expr)
