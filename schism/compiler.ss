...
(library (schism compiler)
  ...
  (define (replace-tail-calls-tail tail)
    (let ((tag (car tail)))
      (cond
       ((or (literal? tail) (eq? tag 'var) (intrinsic? tag)) tail)
       ((eq? tag 'begin)
	(cons tag (replace-tail-calls-tail-begin (cdr tail))))
       ((eq? tag 'if)
	`(,tag ,(replace-tail-calls-expr (cadr tail))
	       ,(replace-tail-calls-tail (caddr tail))
	       ,(replace-tail-calls-tail (cadddr tail))))
       ((eq? tag 'call)
	`(tail-call ,(cadr tail) . ,(map (lambda (e) (replace-tail-calls-expr e)) (cddr tail))))
       ((eq? tag 'apply-procedure)
	`(tail-apply-procedure . ,(map (lambda (e) (replace-tail-calls-expr e)) (cdr tail))))
       ((eq? tag 'let)
	`(,tag ,(map (lambda (binding) `(,(car binding)
					 ,(replace-tail-calls-expr (cadr binding))))
		     (cadr tail))
	       ,(replace-tail-calls-tail (caddr tail))))
       (else (begin (trace-value tail)
		    (error 'replace-tail-calls-tail "unrecognized tail"))))))
  (define (replace-tail-calls-tail-begin tail)
    (cond
     ((null? tail) tail)
     ((null? (cdr tail))
      (cons (replace-tail-calls-tail (car tail)) '()))
     (else (cons (replace-tail-calls-expr (car tail))
		 (replace-tail-calls-tail-begin (cdr tail))))))
  (define (replace-tail-calls-expr expr)
    (let ((tag (car expr)))
      (cond
       ((literal? expr) expr)
       ((eq? tag 'var) expr)
       ((or (intrinsic? tag) (relop? tag))
	(cons tag (map (lambda (e) (replace-tail-calls-expr e)) (cdr expr))))
       ((eq? tag 'call)
	`(,tag ,(cadr expr) . ,(map (lambda (e) (replace-tail-calls-expr e)) (cddr expr))))
       ((eq? tag 'apply-procedure)
	`(,tag . ,(map (lambda (e) (replace-tail-calls-expr e)) (cdr expr))))
       ((eq? tag 'if)
	`(,tag ,(replace-tail-calls-expr (cadr expr))
	       ,(replace-tail-calls-expr (caddr expr))
	       ,(replace-tail-calls-expr (cadddr expr))))
       ((eq? tag 'begin)
	(cons tag (map (lambda (e) (replace-tail-calls-expr e)) (cdr expr))))
       ((eq? tag 'let)
	`(,tag ,(map (lambda (binding) `(,(car binding)
  ...
  (define (compile-exprs exprs env)
    (map (lambda (e) (compile-expr e env)) exprs))
