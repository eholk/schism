...
(library (schism compiler)
  ...
  (define (primitives-rnrs)
    `(library
       ...
       (define (%display-chars-as-string chars i)
         (cond
          ((eq? i (string-length chars)) (begin))
          ((string? chars) (begin (%log-char (string-ref chars i))
                                  (%display-chars-as-string chars (+ 1 i))))
       ...
       (define (make-string n)
         (%set-tag (make-vector n) ,(string-tag)))
       (define (string-length s)
         (if (string? s)
             (vector-length (%set-tag s ,(vector-tag)))
             (error 'string-ref "Invalid arguments")))
       (define (string-ref s i)
         (if (and (string? s)
                  (< i (string-length s)))
             (vector-ref (%set-tag s ,(vector-tag)) i)
             (error 'string-ref "Invalid arguments")))
       (define (string-set! s i c)
         (if (and (string? s)
                  (< i (string-length s))
                  (char? c))
             (vector-set! (%set-tag s ,(vector-tag)) i c)
             (error 'string-ref "Invalid arguments")))
  ...
  (define (add-top-levels defs env)
    (add-top-levels-filter (lambda (name) #t) defs env))
  (define (add-top-levels-filter p defs env)
    (if (null? defs)
        env
  ...
  (define (parse-library lib import-envs)
    ...
    ;;(write "parsing library: ") (display (cadr lib)) (newline)
    (let* ((body (cddr lib))      ;; skip the library and name
           (exports (cdar body))  ;; names of the functions exported
           (imports (cdadr body)) ;; names of libraries imported
           ;;(primitives (primitives))
           (defs (cddr body))
           (env (add-intrinsics (intrinsics)
                                (add-relops (intrinsic-relops) (empty-env))))
           ;;(primitives-env (add-top-levels primitives env))
           (body-env (add-imported (cons
                                    ;; every library implicitly
                                    ;; imports itself
                                    (cadr lib)
                                    imports)
                                   import-envs
                                   env))
           (body-env (add-top-levels-filter (lambda (name)
                                              ;; We want to exclude
                                              ;; exported names, since
                                              ;; those area already in
                                              ;; scope
  ...
  (define (parse-libraries libs)
    (let ((import-envs (map make-export-environment libs)))
      (let ((first (parse-library (car libs) import-envs))
            (imported-functions (fold-left (lambda (functions lib)
                                             (append functions
                                                     (cdr (parse-library lib import-envs))))
                                           '()
                                           (cdr libs))))
  ...
  (define (add-imported imports import-envs env)
    (if (null? imports)
        env
        (begin
          ;;(write "Adding imports from ") (display (car imports)) (newline)
          (add-imported (cdr imports)
                        import-envs
                        (find-import (car imports)
                                     import-envs
                                     env)))))
    ...
    (if (and (pair? expr) (symbol? (car expr)))
        (let ((tag (car expr)))
          (cond
           ...
           ((eq? tag 'char) expr)
           ((eq? tag 'begin)
            (cons 'begin (map lower-strings-expr (cdr expr))))
           ((eq? tag 'var) expr)
           ((eq? tag 'number) expr)
           ...
           ((eq? tag 'call)
            (cons 'call (cons (cadr expr)
                              (map lower-strings-expr (cddr expr)))))
           ((eq? tag 'apply-procedure)
            (cons 'apply-procedure (map lower-strings-expr (cdr expr))))
           ((eq? tag 'if)
            (let ((t (cadr expr))
                  (c (caddr expr))
                  (a (cadddr expr)))
           ...
           ((eq? tag 'icall)
            (cons tag (cons (cadr expr) (map lower-strings-expr (cddr expr)))))
